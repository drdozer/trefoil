...
object AsAuxUnapply {
  implicit def instance[Rep[_, _], S0, S, T](implicit As: Eval.AsAux[Rep[S0, ?], S, T]) = new AsAuxUnapply[Rep, S0, S, T] {
    def apply = As
  }
}
...
object Eval {
  ...
  implicit def evalAppendableToString[S]: AsAux[Const[List[String], ?], S, String] = new As[Const[List[String], ?], S] {
    type T = String
  ...
  : AsAux[Prod[R1, R2, ?], S, (T1, T2)] = new As[Prod[R1, R2, ?], S] {
    type T = (T1, T2)
